...
package com.wuzx.weixin.qyweixin.sdk.msg;
...
import com.yh.qyweixin.sdk.msg.in.event.BatchJob;
...
import com.yh.qyweixin.sdk.msg.in.event.ScanCodeInfo;
import com.yh.qyweixin.sdk.msg.in.speech_recognition.InSpeechRecognitionResults;
...
public class InMsgParser {
	...
	private static InMsg doParse(XmlHelper xmlHelper) {
		...
		log.error("无法识别的消息类型 " + msgType + "，请查阅微信公众平台开发文档");
	...
	private static InMsg parseInEvent(XmlHelper xmlHelper, String toUserName, String fromUserName, Integer createTime,
		...
		if (("subscribe".equalsIgnoreCase(event) || "unsubscribe".equals(event)) && StringUtils.isBlank(eventKey)) {
			return new InFollowEvent(toUserName, fromUserName, createTime, msgType, event, agentId);
		}
		// 扫描带参数二维码事件之一 1: 用户未关注时，进行关注后的事件推送
		if ("subscribe".equalsIgnoreCase(event) && StringUtils.isNotBlank(eventKey) && eventKey.startsWith("qrscene_")) {
